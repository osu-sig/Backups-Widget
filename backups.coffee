#= require ./number_helpers.js

class Dashing.Backups extends Dashing.Widget
  # Make the tile when the page loads
  ready: ->
    super
    backups = $(@node).find(".backups")
    @make_tile(backups.attr('series'))
  ...
  make_tile: (data) ->
    ...
    # remove old graphs
    $(@node).find("#bar").children("g").remove();
  ...
  draw_legend: (variables, options = {}) ->
    ...
    legend = legend_container.selectAll("text")
      ...
      .each((value, index) ->
        ...
        if options.use_value then value_to_use = options.use_value else value_to_use = value.value
        # Draw the value
        g.append("text")
          ...
          .text(NumberHelpers.number_to_human_size(value_to_use, {precision:1, significant: false}))
  ...
  draw_series: (series_data, colors, classes) ->  
    ...
    variables =
      ...
      total: 0
      node: d3.select(@node).select("#bar").append("g")
